...
var AC = (function() {
  ...
  var getAccessTokenFromXACML = function (xacml, success, error) {
    var parser = new x2js.Parser();
    parser.parseString(xacml, function(err,result){
      //Extract the value from the data element
      success(result.Request.Subject[0].Attribute[0].AttributeValue[0]); 
    });
  };

  var checkPolicy = function (roles, xacml, success, error) {
    console.log("Checking authorization to roles", roles, "to do ", action, " on ", path);
    var parser = new x2js.Parser();
    parser.parseString(xacml, function(err,result){
      //Extract the value from the data element
      var XACMLPolicyString = result;
      var subject = [ { 
              '$': { SubjectCategory: 'urn:oasis:names:tc:xacml:1.0:subject-category:access-subject' },
              Attribute: [ {
                '$': { 
      ...
      XACMLPolicyString.Request.Subject = subject;
      for (var i in roles) {
        XACMLPolicyString.Request.Subject[0].Attribute[0].AttributeValue[i] = roles[i]; 
      }
      var js2xml = new x2js.Builder();
  ...
  return {
    getAccessTokenFromXACML: getAccessTokenFromXACML,
