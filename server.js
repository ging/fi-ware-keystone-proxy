...
adminAPI.get('/v2.0/tokens/:token', function(req, res) {
    ...
    if (authDataBase[req.headers['x-auth-token']]) {
        ...
        if (authDataBase[req.params.token].isAdmin) {
            ...
            var access = generateAccessResponse(token, {id: authDataBase[token].tenantId}, authDataBase[token].username, authDataBase[token].username, roles);
            ...
            if (req.headers['accept'] === 'application/xml') {
                ...
                access = generateAccessResponseForXML(token, {id: authDataBase[token].tenantId}, authDataBase[token].username, authDataBase[token].username, roles);
                delete access.access['serviceCatalog'];
